...
sys.path.append('D:/Ml projects/AnimeProject/Anime-Recommendation/ML_Files')
from score_generation import ScoreGeneration
...
class AnicrushBasicDetail:
    ...
    def getDataFrameFromAnimeAPIResponse(self,animeList):
      ...
      animeListDf['fillerScore']=animeListDf['fillers'].apply(ScoreGeneration().fillerPatternScore)
      # Apply the score generation function to each anime
      animeListDf['myanime_comment_score'] = animeListDf['name'].apply(lambda name: sc.animeListCommentsScore(PreProcess().preprocess_Reddit_comments(AniScrappers().fetchReviewFromReddit(name))))
      animeListDf['reddit_comment_score'] = animeListDf['name'].apply(lambda name: sc.animeListCommentsScore(PreProcess().preprocess_MyAnimeList_Comments(AniScrappers().fetchReviewFromMyAnimeList(name))))
      # animeListDf['myanime_comment_score']=animeListDf['name'].apply()
      # animeListDf['reddit_comment_score']= animeListDf['name'].apply()
      return animeListDf
    ...
    def start_And_upload(self):
      print("Entered processing details")
